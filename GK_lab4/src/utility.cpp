...
void util::readLights(std::string name, std::vector<pntLight> *pnts, std::vector<dirLight> *dirs, bool level,
	glm::vec3 *ambient, sunLight *sun)
{
	...
	while (line != "")
	{
		...
		else if (lineType == "pnt")
		{
			pntLight readPnt;
			splitter >> readPnt.color.r >> readPnt.color.g >> readPnt.color.b
				>> readPnt.pos.x >> readPnt.pos.y >> readPnt.pos.z;

			pnts->push_back(readPnt);
		}
		else if (lineType == "dir")
		{
			dirLight readDir;
			splitter >> readDir.color.r >> readDir.color.g >> readDir.color.b
				>> readDir.pos.x >> readDir.pos.y >> readDir.pos.z
				>> readDir.dir.x >> readDir.dir.y >> readDir.dir.z
				>> readDir.focus;

			dirs->push_back(readDir);
		}
...
void util::bindVAO_VTN(unsigned int *VAO, unsigned int *VBO, unsigned int *EBO,
	const float vertices[], unsigned int vertSize, const unsigned int indices[], unsigned int indcSize)
{
	glGenVertexArrays(1, VAO);
	glBindVertexArray(*VAO);
...
glm::mat4 util::myLookAt(glm::vec3 pos, glm::vec3 tgt, glm::vec3 up, bool fallbackRight)
{
	glm::vec3 reDir = glm::normalize(pos - tgt); // kierunek +Z, do tylu
	glm::vec3 right;
	if (fallbackRight && up == reDir) // gracz bezposrednio pod kamera sledzaca
		right = glm::vec3(1.0f, 0.0f, 0.0f);
	else if (fallbackRight && up == -reDir) // gracz bezposrednio nad kamera sledzaca
		right = glm::vec3(-1.0f, 0.0f, 0.0f);
	else
		right = glm::normalize(glm::cross(up, reDir)); // nie zadziala normalizacja jesli up == dir lub -dir
	// chyba to mozna wykorzystywac dla prostych przypadkow,
	// ale zrobic tez funkcje ktora przyjmuje pos, pitch, yaw, roll zeby obliczyc lookAt
	glm::vec3 camUp = glm::cross(reDir, right);
